...
static int replace_variable(struct variables_list *list, char *name,
                            union value value)
{
...
char *get_value(struct variable_item *item)
{
    ...
    return snprintf(item->value.string, VALUE_SIZE, "%d", item->value.integer)
            > 0
        ? item->value.string
        : NULL;
}
...
{
    int res =
        replace_variable(list, "OLDPWD", (union value){ .string = old_pwd });
    return res;
}
...
struct variables_list *init_variables_list(void)
{
    ...
    add_variable(list,
                 init_item("HOME", (union value){ .string = getenv("HOME") },
                           TYPE_STRING));
    ...
    add_variable(
        list,
        init_item("UID", (union value){ .integer = getuid() }, TYPE_INTEGER));
    add_variable(list,
                 init_item("IFS", (union value){ .string = "" }, TYPE_STRING));
    add_variable(
