 ...
 */
struct redirection_node *build_rd_node(int io_number, struct ast *command1,
                                       struct ast *command2) {
 ...
 */
bool check_simple_input(char *input, char rd_char) { return *input == rd_char; }
 ...
 */
struct ast *build_rd_input_node(struct token *token,
 ...
 * check the redirection type input and create the appropriate ast node
 */
struct ast *build_rd_io_node(struct token *token,
                             struct redirection_node *rd_node) {
...
void free_ast_redirection(struct redirection_node *rd_node) {
  // free(rd_node->command1);
  // free(rd_node->command2);
